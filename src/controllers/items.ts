...
router.get("/byUser/:username", (req, res) => {
  getUserByUsername(req.params.username).then(
    (user) => {
      getItemsByUserId(user._id).then(
        (items) => {
          res.write(JSON.stringify({ success: true, items: items }, null, 2));
          res.end();
        },
        (err) => {
          res.json({
            success: false,
            message: `Failed to load items by user ${req.params.username}. Error: ${err}`,
          });
        }
      );
    },
...
router.get("/byCategory/:category", (req, res) => {
  getCategoryByName(req.params.category).then(
    (categoryId) => {
      getItemsByCategory(categoryId, (err, item) => {
        if (err) {
          res.json({
            success: false,
            message: `Failed to get item by category. Error: ${err}`,
          });
        } else {
...
router.post("/", (req, res) => {
  Promise.all([
    getCategoryByName(req.body.category),
    ...
    ([category, user]) => {
      ...
      createNewItem(newItem).then(
          ...
          addItemToUser(newItem, req.body.username, (err) => {
            if (err) {
              ...
              res.json({
                ...
                item: newItem.populate("username"),
              });
            }
          });
        },
        (err) => {
          ...
          res.json({
            success: false,
            message: `Failed to create a new item. Error: ${err}. req: ${req}`,
          });
        }
      );
    },
...
router.get("/getItemsAmountInEachCategory", (req, res) => {
  getItemsAmountInEachCategory().then(
    (result) => {
...
router.put("/:id", (req, res) => {
  itemsModel.findById({ _id: req.params.id }, (err, result: any) => {
    if (err) {
      res.json({
        success: false,
        message: `Failed to find item to update. Error: ${err}`,
      });
    } else {
      getCategoryByName(req.body.category).then(
        (categoryFound) => {
          ...
          result.save((err) => {
            if (err) {
              res.json({
                success: false,
                message: `Failed to save updated item. Error: ${err}`,
              });
            } else {
              res.json({
                success: true,
...
router.delete("/:id", (req, res) => {
  itemsModel.deleteOne({ _id: req.params.id }, (err) => {
    if (err)
      res.json({
        success: false,
        message: `Failed to delete item. Error: ${err}`,
      });
    else {
  ...
  await itemsModel
    ...
    .exec((err, items) => {
      if (err) {
        res.json({
          success: false,
          message: `Failed to load searched items. Error: ${err}`,
        });
      } else {
        res.write(JSON.stringify({ success: true, items: items }, null, 2));
        res.end();
